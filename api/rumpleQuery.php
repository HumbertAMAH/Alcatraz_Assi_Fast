<?php
/* ----------------------------------------------------------
    Alright reserved by Humbert AMAH
    @Copyright (AKH) 2022
    Code updated by Humbert AMAH, date : 2022
------------------------------------------------------------*/
    class RumpleQuery{
        private $dbname = "alcatraz_m";
    ...
    public function auth($table,$tel,$mdp){
			try{
				$req = $this->conn->prepare("SELECT * FROM $table WHERE telephone=? and mdp=? ");
				$req->execute(array($tel,$mdp));
                $res = $req->fetchAll(PDO::FETCH_ASSOC);
